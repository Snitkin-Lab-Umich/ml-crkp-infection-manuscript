...
#------------------------- DEFINE FUNCTION -------------------#
######################################################################
tuning_grid <- function(train_data, model, outcome, hyperparameters, group=NULL){
  ...
  # group is a vector of group assignment for each sample used to split by group
  # during cross-validation using groupKFold
...
#     3. Save the predictions and class probabilities/decision values.
  ...
  if(is.null(group)){
    ...
    cvIndex <- groupKMultiFolds(group, k=folds, times=100)
  }
