...
kg_auc = ggplot(kg$dif, aes(x=auc1,y=auc2)) + geom_point() + 
  geom_abline(intercept=0) +
  geom_hline(yintercept=0.5, lty=3) + 
  geom_vline(xintercept=0.5, lty=3) + 
  ylim(c(0.40,0.8)) + 
...
pk_auc = ggplot(pk$dif, aes(x=auc1,y=auc2)) + geom_point() + 
  geom_abline(intercept=0) +
  geom_hline(yintercept=0.5, lty=3) + 
  geom_vline(xintercept=0.5, lty=3) + 
  ylim(c(0.40,0.8)) + 
...
pdf('results/figures/aucs/aucs.pdf',width=10,height=3)
plot_grid(p1,kg_auc,pk_auc,nrow=1,labels='AUTO',rel_widths = c(0.4,0.3,0.3)) 
dev.off()
